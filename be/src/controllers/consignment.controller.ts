...
export class ConsignmentsController {
  private consignmentsService = new ConsignmentsService(new ConsignmentRepository());
  ...
  public async getOne(req: FastifyRequest<{ Params: { id: string } }>, res: FastifyReply) {
    const consignment = await this.consignmentsService.findById(req.params.id);
    if(res.statusCode === 404) return res.send({ message: "Consignado nao encontrado" });
    return res.send(consignment);
  }

  public async create(req: FastifyRequest<{ Body: Prisma.ConsignmentCreateInput & { userId: string } }>, res: FastifyReply) {
    const consignment = await this.consignmentsService.create(req.body);
    return res.status(201).send(consignment);
  }

  public async update(req: FastifyRequest<{ Params: { id: string }; Body: Prisma.ConsignmentUpdateInput }>, res: FastifyReply) {
    const consignment = await this.consignmentsService.update({ id: req.params.id, data: req.body });
    return res.send(consignment);
  }

  public async delete(req: FastifyRequest<{ Params: { id: string } }>, res: FastifyReply) {
