...
export default function ConsignmentOrderDetailsPage() {
  ...
  const totalItensSent = data.order?.consignmentOrderItems
    .map((item) => item.quantitySent)
    .reduce((a, b) => a + b, 0);
  const totalItensReturn = data.order?.consignmentOrderItems
    .map((item) => item.quantityReturned)
    .reduce((a, b) => a + b, 0);
  const total = totalItensSent - totalItensReturn;
  const { totalValue, totalValueReturn, order } = data;
  ...
  return (
    <div className='max-w-6xl mx-auto space-y-2 px-4'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        <Card className='rounded-2xl py-3 gap-0'>
          <CardHeader>
            <h2 className='text-lg font-semibold text-gray-700'>Itens</h2>
          </CardHeader>
          <CardContent className='divide-y'>
            {order?.consignmentOrderItems?.map((item) => (
      ...
      <DrawnerPayment
        paid={order.paid}
        orderId={order.id}
        totalValue={totalValue - totalValueReturn}
        consignmentOrderItems={order.consignmentOrderItems}
      />
    </div>
  );
}
